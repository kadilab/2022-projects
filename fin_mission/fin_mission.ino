...
void loop() {
   ...
   while(condition == true)
   {
    ...
    if( digitalRead(capteur_feux) == 1)
    {
        ...
        SendSMS("Feux dans la maison");
        calls();
    }
    if(!digitalRead(capteur_fumer)==1)
    {
      Serial.println("Fumer dans la maison");
      SendSMS("Fumer dans la maison");
      calls();
    }
...
void SendSMS(String message)
{
  ...
  delay(100);
  sim800l.print("AT+CMGS=\"+243972688605\"\r");  //Your phone number don't forget to include your country code, example +212123456789"
  delay(500);
